...
function buildQueryURL() {
	...
	var nameEntered = (queryParams.by_name = $("#name").val());
	console.log("Value of Name parameter entered by User: " + nameEntered);

	var stateEntered = (queryParams.by_state = $("#state").val());
	console.log("Value of state parameter entered by User: " + stateEntered);

	var zipEntered = (queryParams.by_postal = $("#zip").val());
	console.log("Value of zip code parameter entered by User: " + zipEntered);

	if (nameEntered > "") {
		queryParams.by_name = $("#name").val().trim();
	}

	if (stateEntered > "") {
		queryParams.by_state = $("#state").val().trim();
	}
...
function updatePage(apiResult) {
	...
	console.log(
		"-----brewData -------called from the Update page function----------------"
	);
